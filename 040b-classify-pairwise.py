...
phone_one = sys.argv[2]
phone_two = sys.argv[3]
this_contrast = '{}_vs_{}'.format(phone_one, phone_two)
...
fname_suffix = cv + nc + this_contrast

# basic file I/O
indir = 'eeg-data-clean'
outdir = op.join(f'processed-data-pairwise{trunc}', 'classifiers')
...
train_mask = np.logical_and(df['train'],
                            np.in1d(df['ipa'], [phone_one, phone_two]))
train_data = data[train_mask]
# make label for phone_one vs phone_two
df['label'] = df['ipa'] == phone_one
...
classifiers['{}-{}'.format(subj_code, this_contrast)] = clf
...
threshold, eer = EER_threshold(clf, X=train_data, y=train_labels,
                               return_eer=True)
eer_fname = op.join(subj_outdir, 'eer-threshold-{}.tsv'.format(this_contrast))
with open(eer_fname, 'w') as f:
    f.write('{}\t{}\t{}\t{}\n'.format('subj', 'contrast', 'threshold', 'eer'))
    f.write('{}\t{}\t{}\t{}\n'.format(subj_code, this_contrast, threshold,
                                      eer))
...
for lang in set(df['lang']):
    ...
    df_out.columns = [phone_two, phone_one]
    df_out[this_contrast] = predictions[lang]
    df_out['lang'] = lang
    fname = 'classifier-probabilities-{}-{}-{}.tsv'.format(lang, fname_suffix,
                                                           subj_code)
    # out_mask = np.in1d(df_out.index, [phone_one, phone_two])
    # df_out = df_out.loc[out_mask]
