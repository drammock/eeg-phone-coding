...
n_feats = 4
plt.ioff()
target = 'presentation'  # presentation or manuscript
    ...
    for x, value in enumerate(row[1:]):  # only plot training / test rows
        if color != bad_color:           # don't draw sparse cells
            ...
            if not np.isnan(value):
                ann = ax.annotate(str(int(value)), xy=(x, y), xytext=(0, -1),
                                  ha='center', fontweight='bold',
                                  size=ticklabelsize + 2, **kwargs)
                if x == (len(row) - 2):
                    saved_labels.append(ann)
