...
# we only care about first ~200ms of brain response, but need to include extra
# pre-zero time to allow later shifting of epochs to align on C-V transition
tmin_baseline = 0.2 - isi_range.min()
tmin = consonant_durs.min() - consonant_durs.max()
tmax = 0.2 + wav_durs.max()
...
for subj_code, subj in subjects.items():
    ...
    epochs = mne.Epochs(raw, events, event_id, tmin, tmax, picks=picks,
                        add_eeg_ref=True, baseline=(tmin_baseline, 0),
                        preload=True)
    # re-align epochs: shift timepoint of stim-start trigger to C-V boundary.
    # Need to do this _after_ baselining.
    # Can't use epochs.crop due to +/-1 sample issues.
    ...
    tmax_cv = tmin_cv + vowel_durs.max()
    epochs_cv = mne.Epochs(raw, events_cv, event_id, tmin_cv, tmax_cv,
                           picks=picks, baseline=None, preload=True)
    epochs_cv._data[:] = np.nan
    for ix, dur in enumerate(consonant_dur_by_trial):
        _tmin = dur - consonant_durs.max()
